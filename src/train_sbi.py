...
from utils.datasets.normal import MultiFFDI_phase1, MultiFFDI_phase1_crop
...
def main(args):
    ...
    train_dataset=MultiFFDI_phase1(phase='train',image_size=image_size, sample_cnt=8000)
    # val_dataset=SBI_Dataset(phase='val',image_size=image_size)
    val_dataset=MultiFFDI_phase1(phase='val',image_size=image_size, sample_cnt=1000)
    ...
    # model_state = torch.load('weights/efficientnet_b4.ra2_in1k.safetensors')
    # loaded = load_file('weights/efficientnet_b4.ra2_in1k.safetensors')
    ...
    now=datetime.now()
    save_path='output/{}_'.format(cfg['model'])+now.strftime(os.path.splitext(os.path.basename(args.config))[0])+'_'+now.strftime("%m_%d_%H_%M_%S")+'/'
    os.mkdir(save_path)
...
if __name__=='__main__':
    ...
    # parser.add_argument('-n',dest='session_name', default='sbi')
