...
class SBI_Dataset(Dataset):
    def __init__(self, phase='train', image_size=224, sample_cnt=None):
        
        assert phase in ['train','val','test']
        
        image_list, label_list = init_multiFFDI_phase1(phase, only_real=True, sample_cnt=sample_cnt)
        
        path_lm='/landmarks/' 
        label_list=[label_list[i] for i in range(len(image_list)) if os.path.isfile(image_list[i].replace('/frames/',path_lm).replace('.jpg','.npy'))]
        image_list=[image_list[i] for i in range(len(image_list)) if os.path.isfile(image_list[i].replace('/frames/',path_lm).replace('.jpg','.npy'))]
    ...
    def __getitem__(self,idx):
        ...
        while flag:
            try:
                ...
                landmark=np.load(filename.replace('.jpg','.npy').replace('/frames/',self.path_lm))[0]
                bbox_lm=np.array([landmark[:,0].min(),landmark[:,1].min(),landmark[:,0].max(),landmark[:,1].max()])
                # bboxes=np.load(filename.replace('.jpg','.npy').replace('/frames/','/retinas/'))[:2]
                # iou_max=-1
...
if __name__=='__main__':
    ...
    from utils.datasets.funcs import IoUfrom2bboxes,crop_face,RandomDownScale
    ...
    image_dataset=SBI_Dataset(phase='val',image_size=256)
    batch_size=64
